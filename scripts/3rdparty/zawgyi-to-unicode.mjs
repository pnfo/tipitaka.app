/**
 * Rules Extract from the Rabbit Converter  
 */

const ZawgyiToUnicodeRules = [
    {
        "from" : "([\u102D\u102E\u103D\u102F\u1037\u1095])\\1+",
        "to" : "$1"
    },
...
function ZawgyiToUnicode(output) {
    return zw2uni_replace_with_rule(ZawgyiToUnicodeRules, output);
}
  
function zw2uni_replace_with_rule(rules, output) {
    rules.forEach(rule => {
        const from_regex = new RegExp(rule["from"], "g");
        output = output.replace(from_regex, rule["to"]);
    });
    return output;
}
